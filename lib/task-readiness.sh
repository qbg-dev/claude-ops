...
# Called by stop-harness-dispatch.sh when it detects a task was marked "completed".
...
TASK_READINESS_ENABLED="${TASK_READINESS_ENABLED:-true}"
TASK_READINESS_MIN_EVIDENCE_CHARS="${TASK_READINESS_MIN_EVIDENCE_CHARS:-50}"
TASK_READINESS_REQUIRED_SECTIONS="${TASK_READINESS_REQUIRED_SECTIONS:-Evidence,Result}"
[ -z "$TASK_READINESS_SECTION_MARKER" ] && TASK_READINESS_SECTION_MARKER='^## '
...
PROGRESS_FILE=$(harness_progress_path "$HARNESS" "$PROJECT_ROOT" 2>/dev/null || echo "")
...
# Parse comma-separated sections into array
IFS=',' read -ra REQUIRED_SECTIONS <<< "$TASK_READINESS_REQUIRED_SECTIONS"
MIN_CHARS="$TASK_READINESS_MIN_EVIDENCE_CHARS"

if [ -n "$BP_FILE" ] && [ -f "$BP_FILE" ]; then
  ARTIFACT_DIR=$(jq -r "${RULES_PREFIX}.verification.artifact_dir // \"claude_files/${HARNESS}-verify\"" "$BP_FILE" 2>/dev/null)
  MIN_CHARS=$(jq -r "${RULES_PREFIX}.verification.min_evidence_chars // $TASK_READINESS_MIN_EVIDENCE_CHARS" "$BP_FILE" 2>/dev/null)
fi
...
for section in "${REQUIRED_SECTIONS[@]}"; do
  if ! grep -q "${TASK_READINESS_SECTION_MARKER}${section}" "$ARTIFACT" 2>/dev/null; then
